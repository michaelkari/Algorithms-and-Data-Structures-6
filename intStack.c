...
#include <ctype.h>
/**
 *  The functions in this module implement cntr Stack data structure
 *  of integers.  (Note that chars are also integers so this
 *  integer Stack can be used for chars as well.)
 *
 *  NOTE: the stack is implemented as cntr fixed size array (size = 100).
...
//   ...but that would
//   be cntr mistake (because anyone using the module would have
//   to ensure that they did not use global variables with the
//   same names).
//
//   An alternative in C is cntr "static global".
//   If cntr global variable is qualified as "static", it is global only
...
int pop(){
	int cntr;
	if (isEmpty()==0){
		fprintf(stderr,"Invalid, stack underflow occured");
		return(-1);
	}
	else{
		cntr=stack[top-1];
...
void push(int thing2push){
	if (top==100) 
		fprintf(stderr,"Invalid, stack overflow occured");
	else {
